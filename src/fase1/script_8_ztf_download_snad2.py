...
#CLASSES_OBJETIVO = ["Delta Scuti", "Irregular"]
CLASSES_OBJETIVO = ["Delta Scuti", "Rotational", "Irregular"]
...
def descargar_curvas_snad_old(df_filtrado, output_dir):
        ...
        try:
            ...
            for object_id, entry in data.items():
                if "lc" not in entry or not entry["lc"]:
                    continue
                df_all = pd.DataFrame(entry["lc"])
                df_all = df_all.rename(columns={"mjd": "tiempo", "mag": "magnitud"})
                df_all["band"] = entry["meta"].get("filter", "unknown")
                df_all["id_objeto"] = id_objeto
                df_all["clase_variable_normalizada"] = clase

                if not df_all.empty:
                    filename = output_dir / f"{id_objeto}_{df_all['band'].iloc[0]}.csv"
                    df_all[["tiempo", "magnitud", "id_objeto", "clase_variable_normalizada", "band"]].to_csv(filename, index=False)
...
def descargar_curva_individual(row, base_url, output_dir, session):
    ra = row["ra"]
    dec = row["dec"]
    id_objeto = row["id_objeto"]
    clase = row["clase_variable_normalizada"]
    ...
    try:
        ...
        return f"⬇ OK: {id_objeto}"

    except Exception as e:
        return f"❌ Error con {id_objeto}: {e}"
...
def descargar_curvas_snad(df_filtrado, output_dir):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        session = requests.Session()
        futures = [
            executor.submit(descargar_curva_individual, row, base_url, output_dir, session)
            for _, row in df_filtrado.iterrows()
        ]
        for f in tqdm(as_completed(futures), total=len(futures), desc="⬇ Descargando curvas ZTF (paralelo)", leave=True):
            try:
    ...
    resumen = Counter([r.split(":")[0] for r in resultados])
    print("\n📊 Resumen de descarga:")
    for key in sorted(resumen):
        print(f"{key:5} → {resumen[key]}")
